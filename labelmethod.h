/**
 * @file labelmethod.h
 * @brief Non recursive method to identify contiguous pixels 
 * @version 0.1
 */
...
void LabelComponent(unsigned short *STACK, unsigned short width,
                    unsigned short height, unsigned char *input, int *output,
...
START: /* Recursive routine starts here */
  ...
  if (output[index] != 0)
    RETURN; /* This pixel has already been labelled  */
...
int LabelImage(unsigned short width, unsigned short height,
               unsigned char *input, int *output)
{
  unsigned short *STACK = (unsigned short *)malloc(3 * sizeof(unsigned short) *
                                                   (width * height + 1));
